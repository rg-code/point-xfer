...
- `scripts/lib/slack.mjs` Slack message for new bonuses: an opener addressed to a random name from `PEOPLE` (general `LINES` with `{name}`, plus `PERSONAL` lines for one person only), then one line per bonus. A bonus announced before its `start` gets a second post (`LIVE_LINES` openers) on the first run once it is live by the US Eastern date (`merge()` returns it in `goingLive` and sets `livePosted` so it posts once); announced on the day, found late or manual → one post. Posted as Rohit by `track-promos.yml` with the `SLACK_USER_TOKEN` secret (user token, scope `chat:write`) to the `SLACK_CHANNEL_ID` variable. Names are plain text, not @mentions, so nobody is pinged. Keep openers harmless; `tests/slack.test.mjs` checks every line names someone on the list.
